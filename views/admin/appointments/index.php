...
<?php $pageTitle = 'MediConnect - All Appointments';
...
<style>
    ...
    .appointment-patient { font-weight:600; color:#0f172a; }
    ...
    .status-approved { background-color:var(--info-bg); color:var(--info-color); }
        ...
        <div class="card-body p-0">
            <?php if (empty($appointments)): ?>
                <div class="empty-state-modern">
                    ...
                    <p class="mb-3">There are no appointments scheduled yet.</p>
                </div>
            <?php else: ?>
                            ...
                            <tbody>
                                <?php foreach ($appointments as $i => $a): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <div class="appointment-patient"><?= h($a['Patient_Name']) ?></div>
                                            <div class="text-muted small"><?= h($a['Visit_Type'] ?? 'General') ?></div>
                                        </td>
                                        <td>
                                            <div class="fw-medium text-dark"><?= h($a['Doctor_Name']) ?></div>
                        ...
                        <nav aria-label="appointments-pagination">
                            <ul class="pagination mb-0">
                                <li class="page-item <?= ($page <= 1) ? 'disabled' : '' ?>">
                                    <a class="page-link" href="<?= (defined('BASE_PATH') ? BASE_PATH : '') ?>/index.php?route=admin/appointments&page=<?= max(1, $page-1) ?>"><i class="bi bi-chevron-left"></i></a>
                                </li>
                                ...
                                for ($p = $start; $p <= $end; $p++): ?>
                                    <li class="page-item <?= ($p === $page) ? 'active' : '' ?>">
                                        <a class="page-link" href="<?= (defined('BASE_PATH') ? BASE_PATH : '') ?>/index.php?route=admin/appointments&page=<?= $p ?>"><?= $p ?></a>
                                    </li>
                                <?php endfor; ?>

                                <li class="page-item <?= ($page >= $totalPages) ? 'disabled' : '' ?>">
                                    <a class="page-link" href="<?= (defined('BASE_PATH') ? BASE_PATH : '') ?>/index.php?route=admin/appointments&page=<?= min($totalPages, $page+1) ?>"><i class="bi bi-chevron-right"></i></a>
                                </li>
                            </ul>
                        </nav>
                    </div>
